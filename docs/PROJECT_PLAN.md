...
| 2026-01-25 | Step 25.2: ASR UX Improvements (Download Progress + Remove pip Guidance) | Done | Improved first-run ASR experience with visible model download progress and removed pip install instructions from desktop builds; Progress schema: added optional "message" field to ProgressSnapshot, added "download_model" stage; Model download detection: added _check_model_cached() to WhisperLocalBackend to detect if model weights exist in ~/.cache/huggingface/hub/; Download progress tracking: modified _load_model() to accept session_id and session_dir parameters, creates ProgressTracker with stage="download_model" when model missing, updates message every 2s during download ("Downloading Whisper model 'base' (~145 MB)... (Xs elapsed)"), marks complete when done; UI progress display: updated session and job detail pages to show message field prominently (gray text below progress bar), hide segment progress during download_model stage, show only elapsed time during download; Removed pip install guidance: replaced all "pip install ..." messages with desktop-friendly text ("This is a packaging error in desktop builds. Download the latest release from the official website."); affected files: transcriber.py (ASR deps), watcher.py (watchdog), claude_client.py (anthropic); Model sizes displayed: tiny (75 MB), base (145 MB), small (490 MB), medium (1.5 GB), large (3.0 GB); 13 new tests: 5 for download progress tracking, 3 for no-pip messaging, 5 for UI message rendering; Does NOT: change packaging, add new backends, bundle model weights |
