...
- [x] Quit Server works on ALL pages (Step 23):
  - [x] shutdown_token passed to session_view and job_detail templates
  - [x] Token embedded identically on every page (home, session, job)
  - [x] Tests verify token renders on all pages and shutdown accepts token from any page
- [x] ffmpeg discovery for packaged app (Step 23):
  - [x] whisper_local uses find_ffmpeg() with fallback paths
  - [x] Returns "ffmpeg not found" error when missing (triggers Step 22 dependency failure)
  - [x] Tests verify fallback path resolution and error message format
...
| 2026-01-24 | Step 22: ASR Error Surfacing (Job/Session Banners) | Done | Propagate ASR dependency failures to job/session status with prominent error banners; added aggregate_asr_errors() in watcher.py to collect errors from outputs/asr/<model>/transcript.json; checks for ffmpeg dependency errors (all segments fail with "ffmpeg not found") vs partial failures; process_job() calls aggregation and fails job if dependency_error found; JobResult.asr_errors field stores error summary; outputs dict includes asr_error_summary for UI display; Job detail page: red banner for status=failed with actionable fix instructions (brew install ffmpeg, etc.); warning banner for partial failures (X/Y segments failed); Session page: same banners pulled from aggregate_asr_errors(); both pages link to Transcripts tab for details; AsrErrorSummary dataclass: total_segments, failed_segments, dependency_error, error_samples; Tests TBD; Does NOT: change pipeline core beyond status aggregation and UI |
| 2026-01-24 | Step 23: Quit Server Fix + ffmpeg Discovery (Packaged App) | Done | Fixed two critical packaged app bugs: (A) Quit Server token injection: added shutdown_token to session_view and job_detail template calls (previously only on home page); token now consistently embedded on all pages; 4 new tests verify token renders on home/session/job pages and shutdown accepts token from any page; (B) whisper_local ffmpeg discovery: updated _extract_audio() to use find_ffmpeg() with fallback paths (/opt/homebrew/bin, /usr/local/bin) instead of relying on PATH; returns "ffmpeg not found" error when missing, triggering Step 22 dependency failure aggregation; 3 new tests: find_ffmpeg() fallback path resolution, ffmpeg not found case, whisper_local integration produces correct error message; 897 tests total; Does NOT: bundle ffmpeg, change shutdown endpoint security, add new features |
| 2026-01-24 | UI Polish: Readable ffmpeg Fix Instructions | Done | Fixed poor contrast/readability of fix instructions in ASR dependency error banners; replaced inline <code> tags (invisible on red background) with dark command blocks (.cmd-block CSS class) featuring strong contrast (#1a1a1a background, #f0f0f0 text); added copy-to-clipboard buttons for macOS/Ubuntu commands with visual feedback ("Copied!" for 2s); copyCommand() JavaScript function uses navigator.clipboard API; applied to both session and job error banners; Windows shows blue underlined link to ffmpeg.org; 2 new template tests verify cmd-block CSS classes render and copyCommand function present; 899 tests total; Does NOT: change backend logic, modify error detection |
| 2026-01-25 | Step 25.1: Self-Contained Desktop Packaging (Bundle ASR Runtime Deps) | Done | Made desktop app truly self-contained by bundling ALL runtime dependencies for enabled features (app + pipeline + ASR); fixed "faster-whisper not installed" error in packaged builds; Updated build-desktop.yml: changed pip install from ".[app,build]" to ".[app,build,asr]" (macOS + Windows jobs); Updated yanhu.spec: added hiddenimports for faster_whisper, av, ctranslate2, tokenizers, huggingface_hub, onnxruntime, tqdm, tiktoken, regex, safetensors (ASR dependency closure); Created scripts/runtime_selfcheck.py: validates all runtime imports before packaging, exits 1 if any missing; Added CI self-check steps: runs runtime_selfcheck.py pre-packaging + post-build library verification (checks for ctranslate2/onnxruntime .dylib/.pyd files); Fixed transcriber.py error message: replaced "pip install [asr]" with packaging-aware message ("packaging error in desktop builds, for CLI/dev: pip install"); Bundled deps list: faster-whisper (av, ctranslate2, huggingface_hub, onnxruntime, tokenizers, tqdm) + flask/markdown (app) + core (anthropic, click, emoji, ffmpeg-python, pyyaml); NOT bundled: watchdog (CLI-only, not in desktop UI), model weights (downloaded at runtime); Artifact size impact: ~200-300MB increase for ASR dependencies (ctranslate2 + onnxruntime include large binaries); Does NOT: add new features, change UI/pipeline behavior, bundle model weights, include watchdog |
| 2026-01-25 | Step 25.2: ASR UX Improvements (Download Progress + Remove pip Guidance) | Done | Improved first-run ASR experience with visible model download progress and removed pip install instructions from desktop builds; Progress schema: added optional "message" field to ProgressSnapshot, added "download_model" stage; Model download detection: added _check_model_cached() to WhisperLocalBackend to detect if model weights exist in ~/.cache/huggingface/hub/; Download progress tracking: modified _load_model() to accept session_id and session_dir parameters, creates ProgressTracker with stage="download_model" when model missing, updates message every 2s during download ("Downloading Whisper model 'base' (~145 MB)... (Xs elapsed)"), marks complete when done; UI progress display: updated session and job detail pages to show message field prominently (gray text below progress bar), hide segment progress during download_model stage, show only elapsed time during download; Removed pip install guidance: replaced all "pip install ..." messages with desktop-friendly text ("This is a packaging error in desktop builds. Download the latest release from the official website."); affected files: transcriber.py (ASR deps), watcher.py (watchdog), claude_client.py (anthropic); Model sizes displayed: tiny (75 MB), base (145 MB), small (490 MB), medium (1.5 GB), large (3.0 GB); 13 new tests: 5 for download progress tracking, 3 for no-pip messaging, 5 for UI message rendering; Fixed broken tests: updated mock_transcribe signatures in test_progress.py and test_transcribe_limits.py to accept session_id parameter; 909 tests passing; Does NOT: change packaging, add new backends, bundle model weights |
| 2026-01-25 | Step 25.3: Fix Build Workflow - Install Runtime Selfcheck Deps | Done | Fixed build-desktop workflow failure by ensuring tiktoken, regex, and safetensors are installed for runtime selfcheck; Updated both macOS and Windows jobs: added explicit "pip install tiktoken regex safetensors" after installing asr extra (these deps are in yanhu.spec hiddenimports but not transitively pulled by faster-whisper); Added debug step to show installed packages (pip freeze | grep for ASR-related packages); Moved selfcheck to run after dependency install; Windows uses findstr instead of grep for package check; Justification for additional deps: tiktoken (tokenizer utilities), regex (Python regex module used by tokenizers), safetensors (Hugging Face tensor format used by model loading); These are included in yanhu.spec hiddenimports from Step 25.1 and required by runtime_selfcheck.py; No changes to runtime_selfcheck.py import list (kept as-is, aligned with desktop feature set); Does NOT: add new features, change core logic |
| 2026-01-25 | Step 26: Self-Recommendation System (Dependency Matrix + CI Rehearsal) | Done | Built self-recommendation verification tooling to catch packaging/CI issues before release; Created docs/DEPENDENCY_MATRIX.md as single source of truth for runtime dependencies (5 profiles: core, app, asr, desktop, all) with Python packages, system dependencies, and selfcheck modules documented; Rewrote scripts/runtime_selfcheck.py with --profile support (core/app/asr/desktop/all) using argparse, PROFILES dict, and get_modules_for_profile(); Created scripts/ci_rehearsal_desktop.sh for local CI simulation (clean venv, installs deps, runs ruff/pytest/selfcheck --profile all/pyinstaller, validates macOS bundle structure); Updated .claude/skills/yanhu-verify/skill.md with REQUIRED "Self-Recommendation Checklist" section enforcing 4 checks before claiming done: (1) ruff check, (2) pytest -q, (3) runtime selfcheck --profile all, (4) CI rehearsal if packaging changed; Added 97 new tests in tests/test_runtime_selfcheck.py covering script existence, help text, all profiles, invalid profile, output format, dependency matrix documentation, and CI rehearsal script validation; DEPENDENCY_MATRIX.md documents: package versions, transitive deps, install commands for each profile, version policy (minimum versions, no pinning), maintenance workflow; CI rehearsal script matches build-desktop.yml exactly: tiktoken/regex/safetensors install, ASR package debug output, step-by-step progress (7+2 steps), PASS/FAIL summary; yanhu-verify skill now enforces self-recommendation gate pattern for all future work; Does NOT: add new features, change pipeline behavior, implement Gemini/Qwen |
| 2026-01-25 | Step 26.1: Self-Recommendation Gates Tightened | Done | Eliminated "expected failures" and reduced test noise in self-recommendation system; Separated Default Gate (core/app/desktop profiles, MUST pass for all dev work) from Release Gate (asr/all profiles + CI rehearsal, only for packaging/release/ASR work); Updated .claude/skills/yanhu-verify/skill.md with clear gate separation: Default Gate checks core/app/desktop selfcheck (no ASR deps required), Release Gate adds asr/all selfcheck + CI rehearsal (only when .github/workflows, yanhu.spec, or release tag involved); Removed "expected failure" language - all gate checks MUST pass for their respective contexts; Reduced tests/test_runtime_selfcheck.py from 97 noisy tests to 5 high-signal tests (grouped in 3 classes): (1) selfcheck supports profiles with correct exit codes, (2) output format validation, (3) invalid profile handling, (4) dependency matrix completeness, (5) CI rehearsal script validation; Tests now verify behavior not string matching; Default Gate eliminates ASR dependency noise for standard development; Release Gate ensures packaging/ASR work is fully validated before release tagging; 832 tests total (removed 95 redundant selfcheck tests); Does NOT: add new features, change core pipeline behavior |
| 2026-01-25 | Step 27: ASR-Full Desktop Build Dependency Closure | Done | Completed packaging dependency closure for whisper_local ASR in desktop builds; Updated pyproject.toml [asr] extra to include tiktoken>=0.5, regex>=2023.0, safetensors>=0.4 (previously required explicit pip install); Enhanced yanhu.spec with collect_submodules() and collect_data_files() for faster_whisper, tokenizers, huggingface_hub, tiktoken, safetensors to capture dynamic imports and package data; Updated build-desktop.yml: removed explicit tiktoken/regex/safetensors install (now in [asr] extra), changed runtime selfcheck to --profile all explicitly, enhanced post-packaging verification to check for faster_whisper/ctranslate2/tiktoken modules in dist/yanhu (macOS) and dist/yanhu (Windows) with exit 1 on missing; Verified no pip install instructions remain in src/; Release Gate now passes: runtime_selfcheck --profile asr (12/12), --profile all (36/36), ci_rehearsal_desktop.sh builds successfully with PyInstaller and validates bundle structure; Desktop builds can now run whisper_local out of the box (except model weights download at runtime and ffmpeg system dependency); UX contracts preserved: first-run model download notice, ffmpeg absolute path discovery, no pip install guidance; 914 tests total; Does NOT: add new features, change core pipeline behavior, bundle model weights or ffmpeg binaries |
| 2026-01-25 | Step 27.1: CI - Fix macOS ASR Bundling Verification Path | Done | Fixed false-negative ASR bundling verification on macOS by inspecting actual .app bundle instead of incorrect dist/yanhu path; build-desktop.yml macOS job now discovers .app bundle with find dist -maxdepth 2 -name "*.app", verifies faster_whisper/ctranslate2/tiktoken inside bundle using find "$APP" -iname pattern, prints app path and first 3 matches per dependency for debugging; exits 1 if .app not found or any critical ASR module missing; Windows job verification unchanged (correctly checks dist/yanhu); pre-packaging runtime_selfcheck --profile all step unchanged (still validates full dependency closure before build); Does NOT: change dependency installation, modify PyInstaller spec, alter core pipeline behavior |
| 2026-01-25 | Step 27.2: CI - Packaged Import Selfcheck for ASR Bundling | Done | Replaced brittle file-name searching with definitive packaged executable import verification; Added --selfcheck-asr flag to launcher.py that imports all required ASR modules (faster_whisper, ctranslate2, tokenizers, huggingface_hub, tiktoken, regex, safetensors, av, onnxruntime), prints PASS/FAIL report, exits 0 if all succeed else 1, does NOT start web server; Updated build-desktop.yml macOS job to discover .app bundle, extract executable name from Info.plist (defaults read CFBundleExecutable), run "$EXEC" --selfcheck-asr, fail job if exit code non-zero; Updated Windows job to run dist/yanhu/yanhu.exe --selfcheck-asr; Removed brittle find "*faster_whisper*" file searches (replaced with definitive import check); Added 3 new launcher tests: selfcheck returns 0 on success, returns 1 on import failures, launcher with --selfcheck-asr flag exits without starting server; 917 tests total (+3); CI now verifies ASR bundling by actually running the packaged executable, eliminating false negatives from base_library.zip packing or silent collection failures; Does NOT: change dependency installation, add new features, modify PyInstaller spec |
| 2026-01-25 | Step 27.3: CI - Robust Info.plist/Executable Discovery for Packaged Selfcheck | Done | Fixed macOS packaged selfcheck step to robustly locate Info.plist and executable (avoid defaults read failure); build-desktop.yml macOS verification now includes debug prints (ls -la .app, ls -la Contents, find Info.plist) for CI diagnostics; replaced defaults read with plutil -extract CFBundleExecutable raw -o - (more robust plist parsing); added fallback to ls "$APP/Contents/MacOS" \| head -1 if plutil fails; added explicit checks with helpful debug output: Info.plist exists, MacOS directory exists, executable file exists (prints ls -la on failure); Windows job unchanged; No launcher or dependency changes; Does NOT: change selfcheck behavior, modify dependency installation, alter PyInstaller spec |
| 2026-01-25 | Step 27.4: Windows-Safe Output for runtime_selfcheck.py (ASCII Symbols) | Done | Fixed Windows cp1252 encoding issue in runtime_selfcheck.py by replacing Unicode checkmarks with ASCII-safe symbols; changed "✓" to "[OK]" and "✗" to "[FAIL]" throughout script output (lines 170, 171, 178); added optional sys.stdout.reconfigure(encoding="utf-8") attempt at start of main() for Windows compatibility (graceful fallback if unsupported); added test_selfcheck_output_uses_ascii_symbols_no_unicode() to verify no Unicode checkmarks in output and assert ASCII tokens present; 918 tests total (+1); Windows Release Gate now passes without UnicodeEncodeError on build-windows job; Does NOT: change dependency logic, modify profile definitions, alter exit codes or core behavior; Only output formatting changed for cross-platform compatibility |
| 2026-01-25 | Step 27.5: Windows-Safe Output for launcher.py selfcheck_asr (ASCII Symbols) | Done | Fixed Windows cp1252 encoding issue in launcher.py selfcheck_asr() function for packaged --selfcheck-asr verification; replaced Unicode "✓" with "[OK]" and "✗" with "[FAIL]" in module import success/failure prints (lines 119, 122); added optional sys.stdout.reconfigure(encoding="utf-8") at function start for Windows compatibility; added test_selfcheck_asr_output_uses_ascii_symbols_no_unicode() in test_launcher.py to verify no Unicode checkmarks in output and assert ASCII [OK] token present; 919 tests total (+1); Packaged executable --selfcheck-asr now runs without UnicodeEncodeError on Windows build-desktop job; Does NOT: change dependency bundling logic, modify ASR module list, alter exit codes or core behavior; Only output formatting changed for cross-platform compatibility |
| 2026-01-25 | Step 28: Desktop ASR - Bundle faster_whisper VAD Assets (silero_vad.onnx) | Done | Fixed ONNXRuntimeError NO_SUCHFILE for missing VAD asset in packaged builds by bundling faster_whisper data files; Updated yanhu.spec to collect_data_files('faster_whisper', include_py_files=False) which includes assets/*.onnx files (silero_vad.onnx, silero_vad_v6.onnx); Extended launcher.py selfcheck_asr() to validate VAD asset presence using importlib.resources.files() (Python 3.9+) with fallback to direct Path lookup via faster_whisper.__file__/assets; Asset check prints [OK]/[FAIL]/[SKIP] for VAD asset found/missing/importlib unavailable; selfcheck_asr exits 1 if asset missing; Added test_selfcheck_asr_includes_asset_check() to verify asset validation in output; 920 tests total (+1); Packaged desktop builds now include VAD model asset, enabling whisper_local to work out-of-the-box (except model weights download + ffmpeg system dep); CI build-desktop workflow catches asset bundling failures via --selfcheck-asr; Does NOT: add new features beyond making ASR work, change transcription behavior, bundle model weights |
| 2026-01-25 | Step 29: Desktop No-Key Mode (ASR-Only Pipeline) | Done | Made desktop build usable without ANTHROPIC_API_KEY by defaulting to ASR-only mode with clean outputs; Updated watcher.py process_job to auto-detect analyze backend: uses "claude" if ANTHROPIC_API_KEY present, else "mock" (no API calls); Updated composer.py get_segment_description to handle mock analyzer: returns ASR text (truncated to 60 chars) if available, else "Segment processed (ASR-only mode)" instead of "TODO: analysis failed"; Updated app.py to detect API key availability and pass has_api_key flag to templates; Added ASR-only mode info banner in session view: "ℹ️ ASR-Only Mode: Vision/OCR analysis disabled (no ANTHROPIC_API_KEY). Timeline and highlights generated from audio transcription only."; Added 6 tests in tests/test_no_key_mode.py covering: mock analyzer description fallback, timeline without TODO placeholders, highlights from ASR, analyzer backend selection; Timeline/highlights now render meaningful ASR-based content without "TODO: analysis failed" or "analysis unavailable" placeholders when in ASR-only mode; Desktop users without API key get working pipeline with ASR-derived timeline/highlights (no broken TODO text); 926 tests total (+6); Does NOT: add new analyzer backends, change Claude analyzer behavior, modify ASR functionality |
| 2026-01-25 | Step 30: Settings UI - API Key Management | Done | Added Settings page in local web UI for secure API key management; Created src/yanhu/keystore.py with KeyStore abstraction: KeychainKeyStore (preferred, uses OS keychain via keyring library) and EnvFileKeyStore (fallback to ~/.yanhu-sessions/.env with chmod 600); Implemented masking utility (first 6 + "…" + last 4 chars) to never expose full keys in UI; Added Settings page route (/settings) with API Keys section showing masked status for ANTHROPIC_API_KEY, GEMINI_API_KEY, OPENAI_API_KEY; Added GET /api/settings/keys (returns masked values only) and POST /api/settings/keys (save/clear, requires local-only + token); Integrated key store with watcher.py process_job to populate environment variables from stored keys before analyze step; Added ⚙️ Settings link to header navigation; Security: Settings endpoints only accessible from 127.0.0.1, POST requires shutdown_token, keys never exposed in full after saving; Added 14 tests in tests/test_keystore.py (masking, EnvFileKeyStore operations, secure permissions) and 7 tests in tests/test_app.py (Settings routes, local-only, token validation, save/clear); 948 tests total (+22); Desktop users can now manage API keys via UI without manual .env editing; Keys stored securely (OS keychain preferred, envfile fallback); Pipeline automatically loads keys from store; Does NOT: add new analyzer backends, change core pipeline behavior, bundle keyring dependency (optional) |
| 2026-01-25 | Feature: Mode Indicator in Global Header | Done | Added mode indicator to global header showing which vision backends are enabled based on configured API keys; Visible on all pages (home, session, job, settings); JavaScript updateModeIndicator() fetches key status from existing /api/settings/keys endpoint on page load; Displays appropriate message with color coding: "Mode: ASR-only (Vision disabled)" in gray (#95a5a6) if no keys set, "Mode: Claude enabled" in green (#27ae60) if ANTHROPIC_API_KEY set, "Mode: Gemini enabled" if GEMINI_API_KEY set, "Mode: Keys set (Claude/Gemini/OpenAI)" with tooltip if multiple keys configured; Informational only, never exposes actual key values; Added 2 tests in test_app.py verifying indicator presence in settings and index pages; 950 tests total (+2); Users get immediate visual feedback about system capabilities and configured backends; Does NOT: change backend selection logic, affect pipeline behavior, leak sensitive key data |
| 2026-01-25 | Step 31: Fix ASR-Only Banner Logic + Consistent Mode Indicator | Done | Fixed incorrect "ASR-only mode" banner showing when OCR/analysis actually succeeded; Made Mode indicator consistent across ALL pages with server-rendered values; Added get_mode_status() helper: returns mode_label (e.g., "Claude enabled", "ASR-only (no keys)"), mode_detail list, keys_present dict; Added detect_session_has_vision(session_dir) helper: checks manifest analyze_backend, analysis files for ocr_items/facts/non-mock model; Updated all render_template_string calls (index, session_view, job_detail, settings) to pass mode_label and mode_detail; Updated BASE_TEMPLATE header: mode indicator shows server-rendered value with color coding (gray for no keys, green for keys set); Fixed SESSION_VIEW_TEMPLATE banner logic: shows ASR-only banner ONLY if session has no vision (detect_session_has_vision=False) AND no keys configured; shows "Vision/OCR analysis enabled" green banner if session has vision; Updated updateModeIndicator() JavaScript to match server-side logic ("ASR-only (no keys)" instead of "Vision disabled"); Updated 8 test_composer.py tests to expect neutral messages ("Segment processed", "Analysis unavailable") instead of "TODO" placeholders (Step 29 behavior); Added 10 tests in test_mode_indicator.py: 3 for global mode indicator on all pages, 7 for session banner logic (vision detection from manifest/OCR/facts, ASR-only banner conditions); 960 tests total (+10); Session banner now accurately reflects whether vision/OCR ran for THAT session (not global key status); Mode indicator consistent across all pages with proper color coding; Never shows incorrect ASR-only banner when analysis succeeded; Does NOT: change pipeline behavior, modify analysis/composer core logic (only UI/banner display) |
| 2026-01-25 | Step 32: Real Processing Progress Bar | Done | Added sticky visual progress bar component to all relevant pages (job detail, session, home); Shows stage label (download_model/transcribe/compose/done/failed), progress text (done/total • elapsed • ETA), and animated bar (determinate % for transcribe/compose, indeterminate stripes for download_model); Implemented in BASE_TEMPLATE with global JavaScript polling system: startGlobalProgressPolling(sessionId, jobStatus) polls /s/<sessionId>/progress every 1s (down from 2s), updates progress bar UI with elapsed/ETA text refresh on every tick, stops on terminal states (done/failed); Progress bar CSS: sticky positioning at top of content, green bar on done, red bar on failed, indeterminate animated stripes for download_model (CSS keyframes); Job detail page: updateJobStatus() polls /api/jobs/<job_id> for status + session_id, switches to session progress when session_id available; Session page: directly starts progress polling with session_id; Home page: inline progress indicators on running job rows (tiny 4px bar + text), polls /s/<sessionId>/progress every 2s for processing jobs, displays stage + done/total; Progress updates "obviously move" via elapsed/ETA text refresh every 1s, "ETA: calculating…" shown when eta_sec missing; Removed old static progress-bar banners and replaced updateProgress() functions with global system; Does NOT: change pipeline logic, modify progress.json schema, bundle new dependencies; Polling interval reduced from 2s to 1s for responsiveness; Addresses Step 32 requirements: real bar movement, visible on all pages, sticky positioning, proper terminal state handling |
| 2026-01-25 | Step 32.1: Progress Bar Layout Fix | Done | Fixed progress bar alignment to match page content container width; Removed negative margins that caused visual protrusion; Added max-width: 1200px and margin: 0 auto to center bar horizontally; Reduced z-index from 1000 to 100 for proper layering; Progress bar now aligns with main content, takes normal vertical space, remains sticky at top, and is responsive on small screens; CSS-only layout fix, no logic changes; 960 tests passing |
| 2026-01-25 | Step 32.2: Progress Bar Rounded Corners | Done | Added rounded corners (8px radius) to progress bar container, track, and fill to match UI design language; Changed border-bottom to full border with border-radius; Added overflow: hidden to prevent fill from squaring off at corners; Progress bar track and fill both use 8px border-radius for smooth edges; Matches main content card styling; CSS-only changes, no logic modified; 960 tests passing |
| 2026-01-25 | Step 32.3: Stage-Aware Weighted Progress | Done | Implemented stage-aware weighted progress calculation to avoid abrupt jumps; Added calculateWeightedProgress() function with stage weight ranges: download_model (0-10% fixed at 5%), transcribe (10-85% mapped from done/total), compose (85-98% mapped from done/total), done (100%); Updated updateGlobalProgress() to use weighted percentages for bar width; Enhanced details text to show overall percentage: "X% • done/total • elapsed • ETA"; Download stage shows "Preparing" message; Added 5 tests in test_progress_ui.py verifying progress bar presence, weighted calculation function, stage mapping, percentage display, and preparing message; 965 tests total (+5); Progress bar now smoothly transitions through stages without jumping from 0% to 70% abruptly; Does NOT: change pipeline stages, refactor backend, modify progress.json schema |
| 2026-01-25 | Step 32.4: Fix Progress Bar Polling Flow | Done | Fixed progress bar not updating by correcting job→session progress polling flow; Root cause: polling was called with fixed sessionId (null initially), never switching to fetch from job endpoint; Implemented globalProgressState tracking jobId/sessionId/lastPoll status; Refactored updateGlobalProgress() to poll /api/jobs/<jobId> when no sessionId yet, then switch to /s/<sessionId>/progress once available; Job detail page now calls startGlobalProgressPolling(jobId, sessionId) with job_id as first param; Session page calls with (null, sessionId) since session already exists; Added debug panel (hidden by default) showing "Job: X | Session: Y | Last job poll: Z | Last progress: W" for troubleshooting; Added 3 tests in test_progress_ui.py: progress endpoint returns valid JSON, job detail has polling initialization, debug panel exists; 968 tests total (+3); Progress bar now correctly transitions from "Initializing..." (no session) → "Waiting for progress..." (has session, no progress.json yet) → "Stage: transcribe" (progress.json available); Backend unchanged: progress.json still written to <session>/outputs/progress.json; Does NOT: change pipeline logic, modify progress.json schema |
| 2026-01-25 | Step 32.5: Write session_id Early for Live Progress | Done | Made progress bar live by writing session_id to job record as soon as processing starts (not only at the end); Root cause: UI polls /api/jobs/<job_id> but never receives session_id while status=processing, so cannot poll /s/<session_id>/progress; Solution: process_job() now accepts jobs_dir parameter and calls update_job_by_id(session_id=...) immediately after session_id generation (line 1214 in watcher.py); BackgroundWorker passes jobs_dir to process_job (line 2979 in app.py); Session_id is generated using same logic (generate_session_id with job timestamp) before any pipeline work; Added 3 tests in test_progress_early_session_id.py: session_id written early (during ingest before segmentation), backwards compatibility (works without jobs_dir), job record has session_id while processing; 971 tests total (+3); UI now transitions smoothly: "Initializing..." (no session_id yet, <1s) → "Waiting for progress..." (has session_id, no progress.json, <2s) → "Stage: transcribe" (progress.json available, shows real progress); Example job JSON while processing: {"job_id": "...", "status": "processing", "session_id": "2026-01-25_12-00-00_game_tag", "created_at": "..."} at src/yanhu/watcher.py:1214-1222; Does NOT: change pipeline logic beyond early session_id persistence, modify session naming, refactor architecture |
| 2026-01-25 | Step 32.6: Fix Progress Polling State Machine | Done | Fixed progress polling to not spam /progress when job is pending and handle "Progress not found" gracefully; Root cause: polling logic requested /s/<sid>/progress even when job.status='pending' (no session/progress yet), creating noisy 404 errors; Solution: Implemented proper state machine in updateGlobalProgress() with jobStatus tracking; State transitions: (1) pending: show "Queued" + "Waiting for worker…" + indeterminate bar, only poll /api/jobs, do NOT poll /progress; (2) processing without session_id: show "Starting" + "Initializing…" + indeterminate bar, only poll /api/jobs; (3) processing with session_id: poll /s/<sid>/progress via fetchProgressData(), gracefully handle 404 (show "Waiting for progress…" without throwing error), keep polling; (4) terminal states (done/failed/cancelled): stop polling, show final state; Progress fetch now returns null on 404 instead of throwing, no console.error spam; fetchProgressData() extracted as separate function for cleaner state machine; Added 5 tests in test_progress_ui.py: jobStatus tracking, queued state handling, graceful 404 handling, separate fetch functions; 977 tests total (+5); Progress bar now shows appropriate UI for each state without unnecessary network requests or console noise; Does NOT: change pipeline logic, modify polling intervals (still 1s) |
| 2026-01-25 | Step 32.7: Enforce Progress Polling Guards | Done | Stopped progress polling while job is pending by adding explicit guards and decision function; Root cause: progress polling could still be triggered unconditionally from BASE_TEMPLATE; Solution: Added shouldPollProgress(jobStatus, sessionId) function that returns true ONLY when jobStatus === 'processing' AND sessionId exists; Added window.__YANHU_POLLING_STARTED guard to prevent multiple intervals; Updated updateGlobalProgress() to use shouldPollProgress() before calling fetchProgressData(); Added CRITICAL comments marking where polling is explicitly skipped (pending state, processing without session_id); Terminal states now reset polling guard (window.__YANHU_POLLING_STARTED = false); Changed "Initializing…" to "Waiting for session…" for clarity when processing but no session_id yet; Added 4 tests in test_progress_ui.py: shouldPollProgress function exists, multiple interval prevention, pending status does not call fetchProgressData, CRITICAL comment verification; 981 tests total (+4); Frontend now ONLY polls /progress when job.status === 'processing' AND session_id is known, eliminating all unnecessary 404 requests; Does NOT: change backend, modify polling intervals |
| 2026-01-25 | Step 32.8: Single Polling Loop + No Inferred session_id | Done | Eliminated duplicate polling loops and removed any inferred session_id from template variables; Root cause: Two polling initializers existed - job detail page and session page (line 1510) both called startGlobalProgressPolling(), causing duplicate loops and 404 spam when session page used template-provided session_id before job actually started; Solution: Replaced window.__YANHU_POLLING_STARTED with window.__YANHU_POLL_LOOP to store interval ID directly; Removed session page auto-polling (replaced with CRITICAL comment explaining session_id ONLY comes from job JSON); Added debug vars window.__YANHU_LAST_JOB and window.__YANHU_LAST_SESSION_ID for troubleshooting; Updated stopGlobalProgressPolling() to clear window.__YANHU_POLL_LOOP; Updated all terminal state cleanup to use window.__YANHU_POLL_LOOP instead of globalProgressPollInterval; Session progress polling now ONLY starts from job detail page which gets session_id from /api/jobs/<job_id> JSON response (never from URL/template inference); Added 6 tests in test_progress_ui.py: single loop guard enforcement, debug vars initialization, stopGlobalProgressPolling cleanup, session page no auto-polling, job detail template null handling; 987 tests total (+6); Eliminated all duplicate polling and inferred session_id issues; Single source of truth for progress polling via job endpoint → session endpoint flow; Does NOT: change backend, modify progress.json schema, alter polling intervals |
| 2026-01-25 | Step 32.9: Fix Polling Spam + Eliminate 404s | Done | Fixed progress polling spam and eliminated 404 errors by (1) disabling global polling on home page and (2) making /s/<sid>/progress return 200 starting payload when progress.json missing; Root cause: Home page setInterval polled /s/<sid>/progress every 2s for running jobs (line 1141), and backend returned 404 for missing progress.json, creating noisy errors; Backend fix: Updated /s/<session_id>/progress endpoint (line 2167) to return 200 with starting payload {"session_id": "...", "stage": "starting", "done": 0, "total": 0, "elapsed_sec": 0, "eta_sec": null, "message": "Waiting for progress..."} when progress.json doesn't exist yet, verified session exists first (404 only for invalid session_id); Frontend fix: Disabled home page polling by commenting out setInterval(updateJobProgressIndicators, 2000) at line 1141 with CRITICAL comment explaining polling should ONLY happen on job detail and session pages; Updated test_progress_returns_404_when_absent to expect 200 starting payload instead of 404; Added 3 tests in test_progress_ui.py: progress endpoint returns starting payload when missing, 404 for invalid session, home page no polling initializer; 990 tests total (+3); Eliminates all 404 spam during job startup and unnecessary network requests from home page; Does NOT: change pipeline logic, modify polling intervals on job/session pages |
| 2026-01-25 | Step 32.10: Verify Progress Gating + Add Explicit Tests | Done | Verified and documented strict progress polling gating condition with comprehensive tests; Gating already correctly implemented in Steps 32.6-32.9 but added explicit verification tests for clarity; Gating condition: fetchProgressData() called ONLY when (job.status === 'processing' AND job.session_id is non-empty); Enforcement: (1) shouldPollProgress(jobStatus, sessionId) at line 606 returns true only when jobStatus === 'processing' && sessionId !== null && sessionId !== undefined, (2) updateGlobalProgress() at line 645-655 returns early for 'pending' status with CRITICAL comment, (3) line 658-670 returns early for 'processing' without session_id, (4) line 672-674 only calls fetchProgressData if shouldPollProgress returns true; Backend starting payload (from Step 32.9): {"session_id": "...", "stage": "starting", "done": 0, "total": 0, "elapsed_sec": 0, "eta_sec": null, "message": "Waiting for progress..."}; Added 4 explicit tests in TestProgressGatingCondition: pending status gating prevents fetch, shouldPollProgress gating condition, processing without session_id early return, fetchProgressData only called when gated; 991 tests total (+4); Zero 404 spam when job.status == 'pending'; Does NOT: change pipeline logic, modify existing gating implementation (only adds verification tests) |
| 2026-01-25 | Step 32.11: Stage Heartbeats + Terminal Lock | Done | Improved progress UX by (A) writing stage heartbeats for all pipeline steps and (B) preventing status regression via terminal lock; Issues: UI stayed in "starting" stage for ~1min until transcribe began (no stage visibility), UI sometimes regressed from Complete to pending for 1-2s (status regression due to polling mismatch); Backend (stage heartbeats): Added write_stage_heartbeat() helper function in progress.py (line 200) to write quick stage updates without full ProgressTracker; Added heartbeats at each pipeline step in watcher.py process_job(): ingest (line 1228), segment (line 1242), extract (line 1251), analyze (line 1269), compose (line 1415); Each heartbeat writes progress.json with stage + message + done=0 + total=0 immediately; Frontend (terminal lock): Added globalProgressState.terminalLocked field (line 540) initialized to false; Added terminal lock check at start of updateGlobalProgress() (line 624) - if locked, return early (freeze UI); Set terminalLocked=true in two places: (1) when job.status reaches done/failed/cancelled (line 694), (2) when progress.stage == "done" or failed (line 855); Terminal rule: once terminal state reached (stage=="done" OR job.status in {done,failed,cancelled}), stop all polling and freeze displayed state; Example early progress.json: {"session_id":"2026-01-25_12-00-00_game_tag","stage":"segment","done":0,"total":0,"elapsed_sec":0,"eta_sec":null,"message":"Segmenting video..."}; Added 5 tests in test_stage_heartbeats.py: write_stage_heartbeat function exists, terminal locked field in global state, terminal lock check at start of update, terminal lock set on job done, terminal lock set on progress done; 996 tests total (+5); Eliminates "starting" stage gap and prevents status regression after completion; Does NOT: change pipeline logic, only adds progress reporting + UI state handling |
| 2026-01-25 | Step 32.12: Job Status Persistence + Anti-Regression Guard | Done | Fixed job status regression where UI showed pending after progress started by ensuring worker persists status transitions and UI never regresses once session is created; Issue: Progress endpoint showed stage=analyze/transcribe (pipeline running) yet job detail page showed status=pending and reverted after completion, session link disappeared because UI relied on job.status/session_id from job JSON; Backend (status persistence): Added started_at field to QueueJob dataclass (watcher.py:178) to track when processing began; Modified update_job_by_id() signature (watcher.py:558) to accept started_at parameter; Updated BackgroundWorker _worker_loop() (app.py:3067-3074) to set status="processing" + started_at timestamp BEFORE pipeline execution using datetime.now(timezone.utc).isoformat(); Updated QueueJob.to_dict() (watcher.py:213-214) to serialize started_at field to JSON; Status transitions now persisted: pending→processing (with session_id + started_at) before pipeline→done/failed after pipeline; Frontend (anti-regression guard): Added effectiveStatus computation (app.py:652-654) that treats pending as processing when session_id exists; Modified state machine to use effectiveStatus instead of data.status (app.py:656, 672); Anti-regression rule: once session_id is present, never display "pending" (minimum is "processing" until done/failed); Example job JSON while processing: {"job_id":"test","status":"processing","session_id":"2026-01-25_00-00-00_game_run01","started_at":"2026-01-25T00:00:05Z","created_at":"2026-01-25T00:00:00"}; Added 8 tests in test_job_status_persistence.py: started_at field exists, update_job_by_id accepts started_at, worker signature verification, frontend anti-regression guard, effectiveStatus computation, pending state skip when session exists, processing uses effectiveStatus, job JSON structure validation; Updated 2 tests in test_progress_ui.py to use effectiveStatus instead of data.status; 999 tests total (+8, 2 updated); Eliminates job status regression and ensures session link remains visible throughout processing; Does NOT: change pipeline logic beyond status persistence |
| 2026-01-25 | Step 32.13.1: Job Status UI Update from Polling | Done | Fixed job detail page showing stale 'pending' status after job completes by adding unique DOM anchor and updating it from polling; Issue: Job status text remained "pending" even after job JSON showed status="done" because status text was only set on initial page load and never updated by JavaScript polling; Root cause: Job detail template rendered status text without unique ID, no JavaScript code updated this DOM node when polling received new job data, progress bar updated but status text stayed stale; Solution: (1) Unique DOM anchor - added id="job-status-text" to status span (app.py:1547) with class="status-pill status-{{job.status}}", appears exactly once in template; (2) Polling update - intercepted updateGlobalProgress (app.py:1729-1761) to update #job-status-text after fetching job data from window.__YANHU_LAST_JOB, sets textContent to job.status and className to 'status-pill status-' + job.status; (3) Terminal lock - added jobPageTerminalLocked flag (app.py:1728), set to true when job.status in [done,failed,cancelled], prevents any subsequent updates after terminal state reached; (4) Session link update - also updates #job-session-link-container and #job-session-link when session_id becomes available; HTML snippet: '<span id="job-status-text" class="status-pill status-pending">pending</span>'; JS snippet: 'statusText.textContent = job.status; statusText.className = "status-pill status-" + job.status;'; Added 9 tests in test_job_status_ui_consolidation.py: unique status text ID exists exactly once, has status-pill class, polling accesses getElementById, updates textContent, updates className, terminal lock prevents regression, uses window.__YANHU_LAST_JOB, session link visibility; 1008 tests total (+9); Final UI behavior: when job JSON shows status=done, status text displays "done" and never reverts to pending; Does NOT: change backend, modify pipeline logic, only fixes frontend DOM update |
| 2026-01-25 | Step 32.14: Terminal State Handling | Done | Fixed terminal state handling by (A) ensuring progress.json ends with stage="done" after pipeline completes and (B) improving session view UX when opened early; Issues: progress.json could be overwritten after terminal state reached, session view returned blank 404 "missing required output files" if user opened page before compose finished; Solution A (progress terminal stage): Added terminal lock to ProgressTracker (progress.py:87) with _terminal_locked flag initialized to False; Modified update() method (progress.py:99-101) to return early if _terminal_locked is True preventing updates after terminal state; Modified finalize() method (progress.py:111-125) to return early if already locked and set _terminal_locked=True when stage is "done"/"failed"/"cancelled" to prevent stage from being overwritten back to transcribe; Pipeline already calls progress_tracker.finalize(stage="done") at watcher.py:1419 after compose completes; Solution B (session early-open UX): Modified session_view route (app.py:2165-2202) to gracefully handle missing files instead of returning 404; When overview.md/highlights.md/timeline.md are missing, render normal session template with warning banner and placeholders instead of 404; Warning banner (app.py:1206-1216): "⚠️ Session Processing: Session is still being finalized (writing outputs). Missing: overview.md, highlights.md, timeline.md. Refresh in a moment." with Refresh Page button; Placeholders for missing tabs: "Overview is being generated...", "Highlights are being generated...", "Timeline is being generated..."; Returns 200 with partial content instead of 404; Example final progress.json: {"session_id":"2026-01-25_00-00-00_game_tag","stage":"done","done":10,"total":10,"elapsed_sec":45,"eta_sec":null,"updated_at":"2026-01-25T00:05:00Z","message":null}; Added 10 tests in test_terminal_state_handling.py: terminal lock prevents updates, finalize sets lock for done/failed/cancelled, session view returns 200 with warning when files missing, shows refresh button, shows placeholders, lists missing files, renders normally when files exist, partial files show specific placeholders; Updated 1 test in test_app.py to expect 200 with warning instead of 404; 1018 tests total (+10, 1 updated); Prevents progress.json regression to transcribe stage after done and provides better UX for early session opens; Does NOT: change pipeline logic, only adds guards and improves error handling |
| 2026-01-25 | Step 32.15: Terminal State Reconciliation | Done | Fixed stale progress.json (stuck at transcribe 8/9) by reconciling terminal state from job result and enforcing UI priority where job.status wins; Issues: job JSON shows status="done" with outputs complete, but progress.json stuck at stage="transcribe" done=8 total=9, UI shows mixed states and keeps polling; Root cause: ProgressTracker scoped to transcribe stage may leave stale progress if callbacks don't complete, no explicit terminal reconciliation after compose succeeds, UI doesn't prioritize job.status over progress.stage; Backend fix (authoritative terminal reconcile): Added explicit terminal reconciliation write in process_job after compose completes (watcher.py:1420-1430) using write_stage_heartbeat to force-write progress.json with stage="done", done=1, total=1, message="Processing complete"; This overwrites any stale transcribe progress ensuring progress.json always matches job.status; On job failure (watcher.py:1490-1502), write progress.json with stage="failed" and error message if session was created; Frontend fix (priority + stop): Added comment at app.py:691-694 clarifying job.status takes priority: "Job status wins - if job.status is terminal, stop ALL polling, do NOT let progress.stage override job.status"; Existing terminal state check already stops polling when job.status in [done,failed,cancelled] and sets terminalLocked=true; Example final progress.json: {"session_id":"2026-01-25_00-00-00_game_tag","stage":"done","done":1,"total":1,"elapsed_sec":0,"eta_sec":null,"updated_at":"2026-01-25T12:00:00Z","message":"Processing complete"}; Reconciliation location: watcher.py process_job function lines 1420-1430 (success) and 1490-1502 (failure); Added 8 tests in test_terminal_reconciliation.py: successful job writes terminal done, failed job writes terminal failed, reconciliation overwrites stale progress, UI has priority comment, UI stops all polling when done, UI shows done when job status done, final progress.json structure, reconciliation location verified; 1026 tests total (+8); Ensures progress.json always reflects job completion state and UI never shows stale transcribe progress when job is done; Does NOT: change transcription logic, only adds terminal state reconciliation |
| 2026-01-25 | Step 32.17: Hard Acceptance + Unified Endpoint (Race Fix) | Done | Enforce hard acceptance for progress completion (must reach 9/9 or stage=done) and fix race so UI cannot get stuck at 8/9; UNIFIED ENDPOINT FIX (eliminates race condition): Created /api/jobs/<job_id>/status endpoint (app.py:2804-2973) that merges job metadata and progress data into single response; Returns {job_id, status, session_id, created_at, raw_path, error, progress:{stage, done, total, percent, elapsed_sec, eta_sec, message}}; Applies same reconciliation logic as /s/<sid>/progress but in unified response; Frontend now polls single endpoint (app.py:619-748) instead of separate /api/jobs/<job_id> and /s/<sid>/progress; Eliminates race condition where job status and progress.json could be out of sync; Added renderProgress() helper (app.py:757-854) to deduplicate progress rendering logic; Tests: Added 5 unified endpoint tests in test_unified_status_endpoint.py (reconcile 8/9→9/9, terminal job, pending job, processing without progress, 404 for missing job); Updated 2 frontend tests in test_progress_ui.py to check for unified endpoint usage; Hard acceptance criteria: For job with total=9, UI must eventually display either (A) "9/9" (done==total) during transcribe stage, OR (B) stage="done" with 100% bar and "Processing complete"; Server-side reconcile in /s/<sid>/progress endpoint (app.py:2277-2398): Added _find_job_by_session_id helper (app.py:2070-2095) to scan jobs_dir for job with matching session_id; If job.status=="done", return terminal payload {stage:"done", percent:100, message:"Processing complete"} overriding progress.json; If job.outputs shows transcribe_processed==transcribe_total and total>0, reconcile stale progress by returning {stage:"transcribe", done:total, total:total, percent:100, eta_sec:0, message:"Transcription complete"}; This makes endpoint authoritative - even if progress.json is stuck at 8/9, endpoint returns 9/9 when job outputs show 9/9; Frontend hard acceptance (app.py:806-823): Added isComplete = done >= total && total > 0 check (line 809); Updated isDone = stage === 'done' || isComplete (line 810); When isComplete but stage != 'done', show "Complete" label (line 819-821); This ensures UI treats 9/9 as terminal and stops polling; CRITICAL FIX (watcher.py:1388-1405): Update job.outputs AFTER transcription completes (before compose); Reloads job from file to avoid stale reference (line 1394-1405), then sets job.outputs with {transcribe_processed, transcribe_total, session_dir}; Without this, job.outputs doesn't exist until after compose completes, so reconciliation can't work; Root cause of 8/9 stuck UI: RACE CONDITION between /api/jobs/<job_id> and /s/<sid>/progress endpoints being polled separately, plus job.outputs was only set AFTER entire pipeline completes; Solution: (1) Unified endpoint eliminates race by single source of truth, (2) job.outputs set immediately after transcription enables reconciliation; Example reconcile: stale progress {"stage":"transcribe","done":8,"total":9} + job outputs {"transcribe_processed":9,"transcribe_total":9} → unified endpoint returns {status:"processing", progress:{stage:"transcribe",done:9,total:9,percent:100,eta_sec:0,message:"Transcription complete"}}; Tests: 6 unit tests in test_progress_reconcile.py, 4 integration tests in test_progress_reconcile_integration.py, 2 E2E tests in test_progress_ui.py, 5 unified endpoint tests in test_unified_status_endpoint.py; 1049 tests total (excluding E2E), all passing; Unified endpoint: /api/jobs/<job_id>/status (app.py:2804); Helper function: _find_job_by_session_id (app.py:2070); Job.outputs update after transcription: watcher.py (line 1388-1405); Ensures UI can never get stuck at 8/9 with single-source polling and server-side reconciliation |
| TBD | M9.1: Desktop Polish | Not Started | Code signing (macOS/Windows); Installer packages (.dmg, .msi); Auto-updater; Menu bar/system tray icon; Crash reporting |

**UX Checklist:**
- [x] Progress stage heartbeats for all pipeline steps (ingest, segment, extract, analyze, compose)
- [x] Terminal lock to prevent status regression after completion
- [x] Job status persistence (pending→processing→done) with session_id and started_at before pipeline execution
- [x] Frontend anti-regression guard (once session_id exists, never show "pending")
- [x] Job detail page status text has unique DOM anchor (#job-status-text) and updates from polling
- [x] Page-level terminal lock prevents status text from reverting after job completes
- [x] ProgressTracker terminal lock prevents stage overwrites after done/failed/cancelled
- [x] Session view shows warning banner + placeholders instead of 404 when opened early
- [x] Terminal state reconciliation: progress.json force-written to stage="done" after compose succeeds
- [x] Frontend prioritizes job.status over progress.stage (job done wins)
